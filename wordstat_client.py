...
_STRONG_TOKENS = [
    ...
    "6t45",
    # конкретные узлы/детали — низкий риск омонимии
    "шток", "втулк", "клипс", "пыльник", "поршен", "гидроблок",
    "гидроаккумулятор", "соленоид", "толкател", "коленвал", "распредвал",
    "шкив", "полумуфт", "картер", "стартер", "спидометр", "редуктор",
    "шестерн", "диск сцеплени",
]
_WEAK_TOKENS = [
...
_OFF_TOPIC_TOKENS = [
    ...
    "робототехник", "специальност", "профессия", "кем работать", "колледж",
    ...
    "гуманоид", "распаковка", "роботех", "r2d2", "трансформер",
    "под прикрытием", "боевой робот", "дитя робота", "вкалывают роботы",
    "робот человек", "робот федор", "робот козел", "робот emo",
    # валюта/курсы
    "usd", "цб рф", "нацбанк", "курс доллар", "курс валют", "банки ру",
    "доллар в рубл", "доллар на сегодня", "рублях на сегодня",
    # кино/сериалы
...
def _is_relevant(phrase: str, seed_words: List[str], allowed_brands: Optional[set]) -> bool:
    ...
    Перед всем этим — проверка _is_off_topic: если фраза несёт явный
    маркер совсем другой темы (учебная специальность, робот в кино,
    валюта, сериалы и т.п.), отсекаем сразу, даже если формально
    совпадает с автомобильным словом (см. комментарий у _OFF_TOPIC_TOKENS
    — Wordstat отдаёт такое прямо в associations самого первого запроса,
...
def _is_strong_relevant(
    phrase: str, strong_seed_words: List[str], allowed_brands: Optional[set]
) -> bool:
    """
    Более строгая версия _is_relevant — используется ТОЛЬКО чтобы решить,
    тратить ли ещё один запрос на расширение вглубь по этой ассоциации, а
    не для решения "оставить фразу в итоговом файле или нет" (для этого
    по-прежнему используется обычная _is_relevant).

    Разница: здесь голого совпадения по _WEAK_TOKENS (вроде "фильтр",
    "крышка", "плита", "сальник", "мехатроник", "робот") НЕДОСТАТОЧНО —
    нужен код платформы/конкретный узел из _STRONG_TOKENS, упоминание
    своей марки, или пересечение с "сильным" (не общим) словом стартовой
    фразы. Это и защищает от истории с "мехатроника" -> "мехатроника и
    робототехника" (учебная специальность) -> "робот человек"/"роботы под
    прикрытием" и т.п. — то есть от третьей(!) стороны значений слова,
    ...
    if _STRONG_RE.search(low):
        return True
    return any(w in low for w in strong_seed_words)

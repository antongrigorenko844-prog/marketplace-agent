...
ВАЖНО — версия ссылки по содержимому файла (?v=<хэш>):
Обнаружено на практике (сентябрь 2026, товар 02e305045): если имя файла
ассета не меняется между перезаливками (а оно у нас как раз стабильное —
"<артикул>_1.png" и т.п.), Ozon один раз скачивает картинку по этой ссылке
и потом ПОВТОРНО ЕЁ НЕ ПЕРЕКАЧИВАЕТ, даже если мы шлём carточку заново с
той же ссылкой и она реально отдаёт уже другое содержимое (проверено:
GitHub Release отдавал новое фото по той же ссылке, хеш содержимого
совпадал с локальным файлом — а на живой карточке Ozon всё равно висело
старое фото). Похоже, Ozon (и, вероятно, WB) кэширует изображение по URL,
а не перепроверяет его содержимое при каждом обновлении карточки.
Чтобы промаркетплейс гарантированно увидел "новую" картинку при смене
содержимого файла, к ссылке добавляется параметр "?v=<первые 10 символов
sha256 от содержимого файла>". Пока содержимое файла не меняется — хэш и
ссылка тоже не меняются (не создаём лишний повод для повторной модерации
на пустом месте). Как только вы кладёте в photos/<артикул>/ файл с другим
содержимым — хэш меняется, ссылка становится новой, и маркетплейс обязан
скачать её заново.
"""
import hashlib
import logging
...
def upload_file(local_path: str, filename: Optional[str] = None, retries: int = 3) -> str:
    ...
    К ссылке добавляется "?v=<хэш содержимого файла>" — см. пояснение в
    начале модуля: без этого Ozon/WB не замечают, что содержимое файла по
    уже известному им URL изменилось, и продолжают показывать старую
    картинку. Хэш меняется только вместе с содержимым файла, так что
    ссылка остаётся стабильной, пока вы не положите в photos/ другой файл.
    """
    token = _get_token()
    repo = _get_repo()
    ...
    for attempt in range(1, retries + 1):
        ...
        if resp.ok:
            base_url = resp.json()["browser_download_url"]
            content_hash = hashlib.sha256(data).hexdigest()[:10]
